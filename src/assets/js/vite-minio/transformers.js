...
const xmlParsers = _interopRequireWildcard(require('./xml-parsers.js'))

const _ = _interopRequireWildcard(require('lodash'))

const _through = _interopRequireDefault(require('through2'))

const _crypto = _interopRequireDefault(require('crypto'))

const _jsonStream = _interopRequireDefault(require('json-stream'))
...
function getErrorTransformer(response) {
  ...
  const headerInfo = {} // A value created by S3 compatible server that uniquely identifies
  // the request.
  ...
  return getConcater(xmlString => {
    const getError = () => {
      ...
      const e = new errors.S3Error(message) // S3 Error code.
...
function getHashSummer(enableSHA256) {
  ...
  const sha256 = _crypto.default.createHash('sha256')
