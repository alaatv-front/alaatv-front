 ...
 */
const crc32 = require('buffer-crc32') // Parse XML and return information as Javascript types
// parse error XML response

function parseError(xml, headerInfo) {
  let xmlErr = {}

  const xmlObj = _fastXmlParser.default.parse(xml)
...
function parseCopyObject(xml) {
  ...
  let xmlobj = (0, _helpers.parseXml)(xml)
...
function parseBucketNotification(xml) {
      ...
      if (filters[0].S3Key) {
        ...
        if (filters[0].S3Key[0].FilterRule) {
          (0, _helpers.toArray)(filters[0].S3Key[0].FilterRule).forEach(rule => {
            const Name = (0, _helpers.toArray)(rule.Name)[0]
            const Value = (0, _helpers.toArray)(rule.Value)[0]
            result.push({
              Name,
              Value
            })
  ...
  if (xmlobj.TopicConfiguration) {
    (0, _helpers.toArray)(xmlobj.TopicConfiguration).forEach(config => {
      const Id = (0, _helpers.toArray)(config.Id)[0]
      const Topic = (0, _helpers.toArray)(config.Topic)[0]
  ...
  if (xmlobj.QueueConfiguration) {
    (0, _helpers.toArray)(xmlobj.QueueConfiguration).forEach(config => {
      const Id = (0, _helpers.toArray)(config.Id)[0]
      const Queue = (0, _helpers.toArray)(config.Queue)[0]
      const Event = genEvents(config.Event)
      const Filter = genFilterRules(config.Filter)
  ...
  if (xmlobj.CloudFunctionConfiguration) {
    (0, _helpers.toArray)(xmlobj.CloudFunctionConfiguration).forEach(config => {
      const Id = (0, _helpers.toArray)(config.Id)[0]
      const CloudFunction = (0, _helpers.toArray)(config.CloudFunction)[0]
      const Event = genEvents(config.Event)
      const Filter = genFilterRules(config.Filter)
...
function parseListParts(xml) {
  ...
  if (xmlobj.Part) {
    (0, _helpers.toArray)(xmlobj.Part).forEach(p => {
      const part = +(0, _helpers.toArray)(p.PartNumber)[0]
...
function parseCompleteMultipart(xml) {
  const xmlobj = (0, _helpers.parseXml)(xml).CompleteMultipartUploadResult

  if (xmlobj.Location) {
    const location = (0, _helpers.toArray)(xmlobj.Location)[0]
    const bucket = (0, _helpers.toArray)(xmlobj.Bucket)[0]
  ...
  if (xmlobj.Code && xmlobj.Message) {
    const errCode = (0, _helpers.toArray)(xmlobj.Code)[0]
    const errMessage = (0, _helpers.toArray)(xmlobj.Message)[0]
    return {
      errCode,
      errMessage
    }
...
function parseListObjectsV2(xml) {
  ...
  if (xmlobj.Contents) {
    (0, _helpers.toArray)(xmlobj.Contents).forEach(content => {
      ...
      const etag = (0, _helpers.sanitizeETag)(content.ETag)
      const size = content.Size
      result.objects.push({
        name,
        lastModified,
...
function parseListObjectsV2WithMetadata(xml) {
  ...
  if (xmlobj.Contents) {
    (0, _helpers.toArray)(xmlobj.Contents).forEach(content => {
      const name = (0, _helpers.sanitizeObjectKey)(content.Key)
      const lastModified = new Date(content.LastModified)
      const etag = (0, _helpers.sanitizeETag)(content.ETag)
      const size = content.Size
      let metadata
...
function parseBucketEncryptionConfig(xml) {
  const encConfig = (0, _helpers.parseXml)(xml)
  return encConfig
}
