...
import webEncoding from 'web-encoding'

const _fs = _interopRequireDefault(fs)

const _http = _interopRequireDefault(http)

const _https = _interopRequireDefault(https)

const _stream = _interopRequireDefault(stream)

const _blockStream = _interopRequireDefault(blockStream2)

const _xml = _interopRequireDefault(xml)

const _xml2js = _interopRequireDefault(xml2js)

const _async = _interopRequireDefault(async)

const _queryString = _interopRequireDefault(queryString)

const _mkdirp = _interopRequireDefault(mkdirp)

const _path = _interopRequireDefault(path)

const _lodash = _interopRequireDefault(lodash)

const _webEncoding = webEncoding

const _helpers = require('./helpers.js')

const _signing = require('./signing.js')

const _objectUploader = _interopRequireDefault(require('./object-uploader'))

const transformers = _interopRequireWildcard(require('./transformers'))

const errors = _interopRequireWildcard(require('./errors.js'))

const _s3Endpoints = require('./s3-endpoints.js')
...
const _extensions = _interopRequireDefault(require('./extensions'))

const _CredentialProvider = _interopRequireDefault(require('./CredentialProvider'))

const _xmlParsers = require('./xml-parsers')

function _getRequireWildcardCache (nodeInterop) {
  if (typeof WeakMap !== 'function') return null
  const cacheBabelInterop = new WeakMap()
  const cacheNodeInterop = new WeakMap()
  return (function (nodeInterop) {
...
class Client {
  constructor(params) {
    ...
    let protocol = ''
    let transport // Validate if configuration is not using SSL
    ...
    const libraryAgent = `MinIO ${libraryComments} minio-js/${Package.version}` // User agent block ends.
  ...
  makeRequest(options, payload, statusCodes, region, returnResponse, cb) {
    ...
    const stream = (0, _helpers.readableStream)(payload)
  ...
  makeRequestStream(options, stream, sha256sum, statusCodes, region, returnResponse, cb) {
    ...
    const _makeRequest = (e, region) => {
      ...
      if (!this.anonymous) {
        ...
        this.checkAndRefreshCreds()
        const authorization = (0, _signing.signV4)(reqOptions, this.accessKey, this.secretKey, region, date)
      ...
      const pipe = (0, _helpers.pipesetup)(stream, req)
      pipe.on('error', e => {
  ...
  getBucketRegion(bucketName, cb) {
    ...
    const extractRegion = response => {
      ...
      let region = _helpers.DEFAULT_REGION;
    ...
    const method = 'GET'
    const query = 'location' // `getBucketLocation` behaves differently in following ways for
  ...
  makeBucket(bucketName, region, makeOpts = {}, cb) {
    ...
    let payload = '' // Region already set in constructor, validate if
  ...
  listIncompleteUploads(bucket, prefix, recursive) {
    ...
    const readStream = _stream.default.Readable({
      objectMode: true
    })
  ...
  getPartialObject(bucketName, objectName, offset, length, getOpts = {}, cb) {
    ...
    const query = _queryString.default.stringify(getOpts)
    ...
    _async.default.waterfall([cb => _fs.default.stat(filePath, cb), (stats, cb) => {
      ...
      if (size <= this.partSize) {
        ...
        (0, _helpers.pipesetup)(_fs.default.createReadStream(filePath, options), hash).on('data', data => {
          ...
          const stream = _fs.default.createReadStream(filePath, options)
      ...
      const uploader = this.getUploader(bucketName, objectName, metaData, multipart) // convert array to object to make things easy
  ...
  putObject(bucketName, objectName, stream, size, metaData, callback) {
    ...
    const chunker = new _blockStream.default({
      size,
      zeroPadding: false
    }) // This is a Writable stream that can be written to in order to upload
    // to the specified bucket and object automatically.

    const uploader = new _objectUploader.default(this, bucketName, objectName, size, metaData, callback) // stream => chunker => uploader
  ...
  listObjectsV2Query(bucketName, prefix, continuationToken, delimiter, maxKeys, startAfter) {
    ...
    const queries = [] // Call for listing objects v2 API
  ...
  statObject(bucketName, objectName, statOpts = {}, cb) {
    ...
    const query = _queryString.default.stringify(statOpts)
  ...
  presignedUrl(method, bucketName, objectName, expires, reqParams, requestDate, cb) {
    ...
    const query = _queryString.default.stringify(reqParams)
  ...
  presignedPostPolicy(postPolicy, cb) {
    ...
    this.getBucketRegion(postPolicy.formData.bucket, (e, region) => {
      if (e) return cb(e)
      const date = new Date()
      const dateStr = (0, _helpers.makeDateLong)(date)
      this.checkAndRefreshCreds()
      ...
      const signature = (0, _signing.postPresignSignatureV4)(region, date, this.secretKey, policyBase64)
  ...
  completeMultipartUpload(bucketName, objectName, uploadId, etags, cb) {
    ...
    const method = 'POST'
    const query = `uploadId=${(0, _helpers.uriEscape)(uploadId)}`
    const parts = []
    ...
    const payload = (0, _xml.default)(payloadObject)
    this.makeRequest({
      method,
      bucketName,
      objectName,
  ...
  getUploader(bucketName, objectName, metaData, multipart) {
    ...
    const multipartUploader = (uploadId, partNumber, ...rest) => {
      ...
      validate(...rest)
      const query = `partNumber=${partNumber}&uploadId=${(0, _helpers.uriEscape)(uploadId)}`
      upload(query, ...rest)
    }
  ...
  getObjectTagging(bucketName, objectName, getOpts = {}, cb = () => false) {
    ...
    this.makeRequest(requestOptions, '', [200], '', true, (e, response) => {
      ...
      (0, _helpers.pipesetup)(response, transformer).on('data', result => {
        tagsList = result
      }).on('error', e => cb(e)).on('end', () => cb(null, tagsList))
    })
  }
