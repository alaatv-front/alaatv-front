<template>
  <div class="donate bg-white q-pa-sm q-mx-sm shadow-4">
    ...
    <div class="row q-my-md text-center reverse-wrap">
      <div class="row col-md-8 col-sm-9 col-xs-12 items-center" style="font-size: 14px">
        <div :class="{activeHelp:!helpAlaa}" class="donate-help col-md-12 col-xs-6 border text-center q-py-sm"
        ...
        <div class="cost row text-center q-mt-md col-md-12 col-xs-6">
          <div v-for="(cost,idx) in donateCost" :key="idx" :class="{activeDonate:cost.isActive}"
               class="cost-donate col border q-mx-xs q-pa-sm" @click="activeDonateCost(idx), helpAlaa=true">
            <span>{{cost.cost}}</span>
          </div>
        </div>
      </div>
