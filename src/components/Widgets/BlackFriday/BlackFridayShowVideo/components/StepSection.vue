    ...
    <div class="items">
      <div v-for="(video, videoIndex) in localBlackFridayCampaignData.videos.list"
        ...
        <q-btn @click="onSelectStep(videoIndex)">
          <template v-if="hasImageIcon(videoIndex)">
...
export default defineComponent({
  ...
  methods: {
    ...
    hasImageIcon (videoIndex) {
      const stepIcon = this.stepsIconList.filter(icon => parseInt(icon.index) - parseInt(this.startIndex) === videoIndex)
      return stepIcon.length > 0
    },
    getIconSrc (videoIndex) {
      const imageIcon = this.stepsIconList.filter(icon => parseInt(icon.index) - parseInt(this.startIndex) === videoIndex)[0]
      return imageIcon.src
    }
  }
})
</script>
