<template>
  <div class="shadow-5 bg-white q-pa-md q-mb-lg rounded-borders content-info">
    <h6 class="set-title">
      {{content.title}}
    </h6>
    <q-tabs>
      <q-tab name="توضیحات" label="توضیحات"/>
    </q-tabs>
    <div v-if="content.author">
      {{content.author.first_name}} {{content.author.last_name}}
    </div>
    <h6 v-if="content.set" class="set-title">
      {{content.set.title}}
    </h6>
    <div v-html="content.inputData.body"></div>
    <q-separator class="q-my-lg"/>
    <q-separator class="q-my-lg"/>
    <div v-if="content.tags" class="row">
      <p class="col-1 q-mt-sm text-center">تگ ها</p>
      <div class="col q-pl-sm">
        <q-badge v-for="badge in content.tags" :key="badge" class="q-px-sm q-ml-sm" color="blue">
          {{badge}}
        </q-badge>
      </div>
    </div>
