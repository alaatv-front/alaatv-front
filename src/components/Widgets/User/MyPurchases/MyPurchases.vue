    ...
    <div class="col-md-4 col-sm-12 productsCol q-pa-sm-sm q-pa-xs-xs">
      <div class="q-px-xs-none q-pa-sm-md">
        ...
        <router-link :to="{name: 'UserPanel.Asset.Abrisham.Progress'}"
                     class="q-mb-md">
          <div class="abrishamAssetBanner">
            <img alt="داشبورد ابریشم پرو"
                 src="https://nodes.alaatv.com/upload/banner-dashboard-abrishampro.jpg">
          </div>
        </router-link>
...
export default {
  ...
  methods: {
    ...
    getPurchasedProducts () {
      this.loading = true
      this.$apiGateway.user.getPurchasedProducts().then((res) => {
        this.products = res.referralCodeList
        this.loading = false
      })
    },
    filterProduct () {
