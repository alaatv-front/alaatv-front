<template>
  <div>
    product item
  </div>
<!--  <div-->
<!--    class="product-item-box {{ isset($class) ? $class : '' }}"-->
<!--    productId="{{$product->id}}"-->
<!--  >-->
<!--    <div class="img-box">-->
<!--      <a data-product-id="{{$product->id}}" class="a&#45;&#45;gtm-eec-product a&#45;&#45;gtm-eec-product-click"-->
<!--         @include('partials.gtm-eec.product', ['position'=>$productKey, 'list'=> isset($list) ? $list : '-', 'product'=>$product])-->
<!--      href="{{ $product->url }}" target="_blank">-->
<!--      <img-->
<!--        src="https://nodes.alaatv.com/loder.jpg?w=1&h=1"-->
<!--        data-src="{{$product->photo}}"-->
<!--        class="lazy-image img"-->
<!--        alt="product"-->
<!--        width="1"-->
<!--        height="1"-->
<!--      />-->
<!--      </a>-->
<!--    </div>-->
<!--    <div class="product-item-main-box">-->
<!--      <div class="product-item-content">-->
<!--        <div class="main-title">-->
<!--          <a data-product-id="{{$product->id}}" class="title-box a&#45;&#45;gtm-eec-product a&#45;&#45;gtm-eec-product-click"-->
<!--             @include('partials.gtm-eec.product', ['position'=>$productKey, 'list'=> isset($list) ? $list : '-', 'product'=>$product])-->
<!--          href="{{$product->url}}" target="_blank">-->
<!--          <span class="title-text">-->
<!--                     {{ $product->name }}-->
<!--                    </span>-->
<!--          </a>-->
<!--        </div>-->
<!--        <div class="price-box">-->

<!--          <div class="price-info">-->
<!--            @if($product->price['final'] !== $product->price['base'])-->
<!--            <div class="discount">-->
<!--                            <span>-->
<!--                                %{{ round((1-($product->price['final']/$product->price['base']))*100) }}-->
<!--                            </span>-->
<!--            </div>-->
<!--            <div class="main-price">{{number_format($product->price['base'])}}</div>-->
<!--            @endif-->
<!--            <div class="final-price-box">-->
<!--              <div class="final-price">-->
<!--                {{ number_format($product->price['final']) }}-->
<!--              </div>-->
<!--              <div class="price-Toman">-->
<!--                تومان-->
<!--              </div>-->
<!--            </div>-->
<!--          </div>-->
<!--        </div>-->
<!--        <div class="action-box">-->
<!--          <div class="more-detail product-more-detail">-->
<!--            <a class="a&#45;&#45;gtm-eec-product a&#45;&#45;gtm-eec-product-click"-->
<!--               @include('partials.gtm-eec.product', ['position'=>$productKey, 'list'=> isset($list) ? $list : '-', 'product'=>$product])-->
<!--            data-product-id="{{$product->id}}"-->
<!--            href="{{ $product->url }}" target="_blank">-->
<!--            <span>توضیحات</span>-->
<!--            <span class="more">بیشتر</span>-->
<!--            </a>-->
<!--          </div>-->
<!--          @if(!isset($disableAddToCartBtn) || $disableAddToCartBtn === false)-->
<!--          <button-->
<!--            productId="{{$product->id}}"-->
<!--            data-product-id="{{$product->id}}"-->
<!--            data-href="{{ $product->url }}"-->
<!--            class="btn-style add-product-btn"-->
<!--            @include('partials.gtm-eec.product', ['position'=>$productKey, 'list'=> isset($list) ? $list : '-', 'product'=>$product])-->
<!--          >-->
<!--          <img-->
<!--            src="https://nodes.alaatv.com/upload/landing/28/productSection/landing-taftan-product&#45;&#45;section-add-square.png" />-->
<!--          ثبت نام-->
<!--          </button>-->
<!--          @endif-->
<!--          {{&#45;&#45;                <button&#45;&#45;}}-->
<!--          {{&#45;&#45;                    class="btn-style active hide"&#45;&#45;}}-->
<!--          {{&#45;&#45;                >&#45;&#45;}}-->
<!--          {{&#45;&#45;                    <div class="content">&#45;&#45;}}-->
<!--          {{&#45;&#45;                        <img&#45;&#45;}}-->
<!--          {{&#45;&#45;                            src="https://nodes.alaatv.com/upload/landing/28/productSection/landing-taftan-product&#45;&#45;section-tick-square.png"/>&#45;&#45;}}-->
<!--          {{&#45;&#45;                        اضافه شد&#45;&#45;}}-->
<!--          {{&#45;&#45;                    </div>&#45;&#45;}}-->

<!--          {{&#45;&#45;                </button>&#45;&#45;}}-->
<!--        </div>-->

<!--      </div>-->
<!--    </div>-->
<!--  </div>-->
</template>

<script>

</script>

<style lang="scss" scoped>

.product-item-box {
  margin-bottom: 25px;
  position: relative;
  border-radius: 20px;
  box-shadow: -2px -4px 10px rgba(255, 255, 255, 0.6), 2px 4px 10px rgba(46, 56, 112, 0.05);
  background-color: #ffffff;
}

.product-item-box .product-item-main-box {
  padding: 10px 16px 14px 16px;
}

.product-item-box .img-box .img {
  border-radius: 20px 20px 0 0;
}

.product-item-box .img-box * {
  width: 100%;
}

/*----------------title--------------*/
.product-item-box .product-item-main-box .product-item-content .main-title {
  margin-bottom: 2px;
}

.product-item-box .product-item-main-box .product-item-content .main-title a {
  margin-bottom: 0;
}

.product-item-box .product-item-main-box .product-item-content .main-title .title-box {
  display: flex;
  align-items: start;
  width: 100%;
  height: 24px;
}

.product-item-box .product-item-main-box .product-item-content .main-title .title-box .title-text {
  font-weight: 500;
  font-size: 14px;
  line-height: 24px;
  text-align: right;
  letter-spacing: -0.03em;
  color: #333333;
  display: -webkit-box;
  -webkit-line-clamp: 1;
  -webkit-box-orient: vertical;
  text-overflow: ellipsis;
  overflow: hidden;
}

/*-------------------img box----------------------*/
.product-item-box .img-box {

  max-height: 260px;
}

.product-item-box .img-box .img {
  max-height: 260px;
}

/*-------------------price box----------------------*/
.product-item-box .product-item-main-box .product-item-content .price-box {
  display: flex;
  flex-wrap: nowrap;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 13px;
}

.product-item-box .product-item-main-box .product-item-content .price-box .add-cart-info {
  display: flex;
  justify-content: center;
  align-items: center;
}

.product-item-box .product-item-main-box .product-item-content .price-box .price-info {
  display: flex;
  justify-content: center;
  align-items: baseline;
}

.product-item-box .product-item-main-box .product-item-content .price-box .price-info .final-price-box {
  display: flex;
  justify-content: center;
  align-items: center;
}

.product-item-box .discount {
  width: 36px;
  height: 22px;
  border-radius: 6px 6px 6px 0;
  background-color: #E05555;
  display: flex;
  justify-content: center;
  align-items: center;
  margin-left: 5px;
}

.product-item-box .discount span {
  color: white;
  font-weight: 500;
  font-size: 14px;
  padding-top: 3px;
}

.product-item-box .product-item-main-box .product-item-content .price-box .price-info .main-price {
  color: #ff5050;
  text-decoration: line-through;
  margin-left: 12px;
  font-size: 12px;
}

.product-item-box .product-item-main-box .product-item-content .price-box .price-info .final-price-box .final-price {
  margin-left: 4px;
  font-size: 16px;
  font-weight: 500;
  line-height: 28px;
}

.product-item-box .product-item-main-box .product-item-content .price-box .price-Toman {
  font-size: 10px;
  font-weight: 500;
  line-height: 17px;
}

.product-item-box .product-item-main-box .product-item-content .price-box .add-cart-info .add-cart-icon {
  width: 19px;
  order: 1;
}

/*------------------- action-box ----------------------*/
.product-item-box .product-item-main-box .product-item-content .action-box {
  display: flex;
  justify-content: space-between;
  align-items: center;
}

.product-item-box .product-item-main-box .product-item-content .action-box .more-detail {
  font-weight: 500;
  font-size: 12px;
  line-height: 21px;
  color: #666666;
  cursor: pointer
}

.product-item-box .product-item-main-box .product-item-content .action-box .more-detail a {
  text-decoration: none;
  color: inherit;
}

.product-item-box .product-item-main-box .product-item-content .action-box .btn-style {
  width: 116px;
  height: 32px;
  background-color: #4CAF50;
  border-radius: 10px;
  border: none;
  color: white;
  font-size: 14px;
  letter-spacing: -0.03em;
}

.product-item-box .product-item-main-box .product-item-content .action-box .btn-style .content{
  display: flex;
  align-items: center;
  justify-content: center;
}

.product-item-box .product-item-main-box .product-item-content .action-box .btn-style img {
  width: 15px;
  height: 15px;
  margin-left:7px;
}

.product-item-box .product-item-main-box .product-item-content .action-box .active {
  border: 2px solid #4CAF50;
  color: #4CAF50;
  background-color: white;
}

@media screen and (max-width: 992px){
  .product-item-box .img-box {

    max-height: 230px;
  }

  .product-item-box .img-box .img {
    max-height: 230px;
  }

  .product-item-box .product-item-main-box .product-item-content .action-box .btn-style {
    width: 102px;
  }
  .product-item-box .product-item-main-box .product-item-content .action-box .btn-style img {
    margin-left: 0;
  }
}
@media screen and (max-width: 768px){
  .product-item-box .img-box {
    max-height: 241px;
  }

  .product-item-box .img-box .img {
    max-height: 241px;
  }
}

@media screen and (max-width: 575px) {
  .landing-content-width {
    padding-left: 11px;
    padding-right: 11px;
  }

  .product-item-box {
    min-height: 120px;
    max-height: 120px;
    display: flex;
    border-radius: 18px;
    margin-bottom: 16px;
  }

  .product-item-box .img-box {
    min-width: 120px;
    min-height: 120px;
    max-width: 120px;
    max-height: 120px;
  }

  .product-item-box .img-box .img {
    border-radius: 0 18px 18px 0;
    min-width: 120px;
    min-height: 120px;
    max-width: 120px;
    max-height: 120px;
  }

  .product-item-box .product-item-main-box {
    padding: 10px 12px 10px 12px;
    width: 100%;
  }

  .action-box .more-detail .more {
    display: none;
  }

  .product-item-box .product-item-main-box .product-item-content .action-box .btn-style {
    width: 100px;
    height: 30px;
    border-radius: 8px;
  }

  .product-item-box .product-item-main-box .product-item-content .action-box .btn-style .tick {
  }

  .product-item-box .discount {
    height: 20px;
    margin-left: 3px;
  }

  .product-item-box .product-item-main-box .product-item-content .main-title .title-box {
    height: 44px;
    justify-content: center;
  }

  .product-item-box .product-item-main-box .product-item-content .main-title {
    margin-bottom: 0;
  }

  .product-item-box .product-item-main-box .product-item-content .main-title .title-box .title-text {
    -webkit-line-clamp: 2;
  }

  .product-item-box .product-item-main-box .product-item-content .price-box {
    margin-bottom: 0;
  }

  .product-item-box .product-item-main-box .product-item-content .price-box .price-info .main-price {
    margin-left: 4px;
  }

  .product-item-box .product-item-main-box .product-item-content .price-box .price-info .final-price-box .final-price {
    margin-left: 2px;
  }
}

</style>
