      ...
      <q-card-section class="comment-footer">
        ...
        <div class="comment-">
          <q-btn color="primary"
                 ...
                 :to="{name: 'UserPanel.Asset.ChatreNejat.Content', params: {productId: $route.params.productId, setId: comment['set'].id, contentId: comment.content.id}}" />
        </div>
      </q-card-section>
    </q-card>
  </div>
  ...
  methods: {
    ...
    deleteComment() {
      this.$apiGateway.comment.delete({ data: { id: this.$route.params.commentId } }).then(() => {
        this.$router.push({ name: 'UserPanel.Asset.ChatreNejat.ProductComments', params: { productId: this.$route.params.productId } })
      }).catch(() => {})
    },
    loadData(productId) {
